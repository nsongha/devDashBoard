...
- Parsers và collectors → đề xuất test vì dễ có edge cases
- UI thuần / CSS changes → KHÔNG cần nhắc test
...
- Verify syntax: `node --check src/server.mjs && node --check collect.mjs`
- Verify server start được bình thường
