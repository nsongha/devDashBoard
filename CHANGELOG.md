...
- **Flexible parser formats**: Known Issues hỗ trợ heading format `### ISSUE-xxx:` (không brackets), severity sections (🔴 Critical/🟠 High/🟡 Medium/🟢 Low), `**Severity**`/`**Status**`/`**Affects**` fields. QC Report hỗ trợ table format (`| File | Status |` với ✅/❌), numbered headings (`## 1. Compile Check`), unicode checkboxes (□), section `Kết quả` fallback cho sign-off
